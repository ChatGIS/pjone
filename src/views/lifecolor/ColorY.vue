...
const initCalendar = async () => {
  ...
  const data = []
  const res = await lifeColorApi.getLifeColorList({
    current: 1,
    size: 200,
    type: 'Y'
  })
  ...
  for (let i = 0; i < records.length; i++) {
    if (records[i].type == 'YN') {
      data.push([records[i].doDate, '0'])
    } else if (records[i].type == 'YY') {
      data.push([records[i].doDate, '1'])
    }
    if (records[i].type == 'YMS' || records[i].type == 'YME') {
      recordYM.push(records[i])
    }
  }
  for (let i = recordYM.length - 1; i >= 0; i = i - 2) {
    const start = recordYM[i].doDate
    const end = recordYM[i - 1] ? recordYM[i-1].doDate : formatDate(new Date())
    const range = getDatesBetween(start, end)
    for (let j = 0; j < range.length; j++) {
      const index = getDayIndex(range[j], data)
      if (index != -1) {
        data[index][1] += '|2'
      } else {
        data.push([range[j], '2'])
  ...
  const colors = [colorYN, colorYY, colorYM]
  option = {
    tooltip: {
      // 鼠标悬浮提示
      position: 'top',
      formatter: function (params) {
        let showValue = params.value.toString()
        return showValue
      }
    },
    calendar: [
